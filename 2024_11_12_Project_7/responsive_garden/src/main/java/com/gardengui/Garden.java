...
public class Garden {
    ...
    public int getRows() {
        return plotsList2D.size(); // Return the number of rows
    }

    public int getColumns() {
        // Return the number of columns in the first row (assuming all rows have the same number of columns)
        return plotsList2D.isEmpty() ? 0 : plotsList2D.get(0).size();
    }

    public Plot getPlot(int row, int col) {
        return plotsList2D.get(row).get(col);
    }
